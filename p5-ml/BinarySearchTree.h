...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node *copy_nodes_impl(Node *node) {
    if(empty_impl(node)){
      ...
      Node *node_copy = new Node(node->datum,
                      node->left,node->right);
      node_copy->left =  copy_nodes_impl(node_copy->left);
      node_copy->right =  copy_nodes_impl(node_copy->right);
      return node_copy;
    }
 ...
 static bool check_sorting_invariant_impl(const Node *node, Compare less) {
  ...
  if(empty_impl(node)){
    ...
    if(min_element_impl(node -> right) == nullptr){
      ...
      check_right = false;
      check_right = less(node->datum, min_element_impl(node -> right)->datum);
    ...
    if(max_element_impl(node -> left) == nullptr){
      ...
      check_left = false;
       check_left = less(max_element_impl(node -> left)-> datum, node->datum);
    } 
  ...
  // EFFECTS : Traverses the tree rooted atx 'node' using an in-order traversal,
  //           printing each element to os in turn. Each element is followed
  //           by a space (there will be an "extra" space at the end).
  //           If the tree is empty, nothing is printed.
  // NOTE: This function must be tree recursive.
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    if(empty_impl(node)){
      ...
      Node *node_copy=min_greater_than_impl(node->left,val,less);
      if(node_copy){
        return node_copy;
      }else{
        return node;
  ...
  //       while(!empty_impl(node_copy->left)){
